...
def roman_to_int(s:str) -> int:
    ...
    for i in range(len(s)):
        ...
        if i + 1 < len(s) and value < roman_to_int_dict[s[i+1]]:
            total -= value
        else:
            total += value 
    return total 
...
# Approach
# Create a dict to map Roman numerals (I, V, X, etc.) to their integer values.
# Initialize a variable result to store the final integer value.
# Loop through the string:
#   If the current numeral is smaller than the next numeral and its not the last element, subtract its value from result.
